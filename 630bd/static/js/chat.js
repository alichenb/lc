...
(function () {
    ...
    this.showPay = function (p) {
        ...
        dlg.hideTitle();
        dlg.setArgs(p);
        dlg.showPage("index.php?m=Index&c=Pay&a=index", 900, 620);
    };
    var getUrl = function () {
        user = cm.getUser();
        uid = user['id'];
        // chatServer = "ws://122.114.89.168:2346/?uid="+uid;
        chatServer = "ws://47.74.15.155:2346";
    ...
    var initEmotion = function () {
        ...
        var stickerList = [{
            capacity: 18,
            des: "DADA",
            dir: "./Public/Index/Images/dada/",
            name: "DADA"
        }].concat(cm.getSticker());
    ...
    var initChatEvent = function () {
        ...
        var chat_packet = cm.get("chat_packet");
        var packet = cm.get("packet_object");
        var zj = cm.get("packet_zj");
        var chat_gift = cm.get("chat_gift");
        chat_close.onclick = function () {
            setDisplay(!chat_show.isVisible());
        };
        //把合并的三个函数 分来了 防止影响到其他的点击事件
        head_chat.onclick = short_chat.onclick = function () {
            setDisplay(!chat_show.isVisible());
        }
        //        head_chat.onclick = short_chat.onclick = function(){
        ...
        chat_packet.onclick = function (e) {
            e = e || window.event;
            var target = e.target || e.srcElement;
            var address = curContact['user_id'];
            //console.log(address);
            if (target.nodeName.toLowerCase() == "div") {
                if (!packet.style.display) {
        ...
        zj.onclick = function () {
            var packet_val = cm.get("packetValue").val();
            var address = curContact['user_id'];
            if (packet_val < 1) {
                alert("> 1 Gold");
            } else if (cm.getUserGold() - packet_val <= 0) {
                alert(cm.i18n("need_recharge"));
            } else {
                cm.ajax("index.php?m=Index&c=Chat&a=zengsong", {
                    "address": address,
                    "packet": packet_val
                }, function (res) {
        ...
        chat_gift.onclick = function () {
            var username = curContact['username'];
            var user_id = curContact['user_id'];
            localStorage.setItem("gift_mask", 1);
    ...
    var initTrans = function () {
        ...
        var other_language = cm.get(new Option());
        name.html(cm.i18n("trans_service"));
        name.val(-1);
        sel.append(name);
        other_language.html(cm.i18n("other_language"));
        other_language.val("zh-cn");
        if (user.inner_member != 1) {
            // other_language.selected = true;
        }
        sel.append(other_language);
        //gog.label = "Google";
    ...
    var doHeart = function () {
        doReq(100);
        heartTimer = setTimeout(doHeart, 1000);
    };
            ...
            switch (opt) {
                case 0:
                    ...
                    if (hasInit == 0) {
                        hasInit = 1;
                        des = "建立链接+加载联系人";
                        window.console.log(res["contact"]);
    ...
    var createRItem = function (member) {
        ...
        var cBar = function () {
            ...
            favor.onclick = function () {
                cm.ajax("index.php?m=Index&c=First&a=favor", {
                    favorId: member["id"]
                }, function (res) {
                    if (res["status"] == 'success') {
                        alert("ok");
            ...
            like.onclick = function () {
                cm.ajax("index.php?m=Index&c=First&a=like", {
                    likeId: member["id"]
                }, function (res) {
                    if (res["status"] == 'success') {
                        alert("ok");
    ...
    var createContact = function (c) {
        ...
        life.html("&bull;");
        if(c['is_online'] == 0) {
            name.html('<span style="color:grey">●&nbsp;&nbsp;</span>' + c["username"]);
        } else {
            name.html('<span style="color:green">●&nbsp;&nbsp;</span>' + c["username"]);
        }
